...
| Conversational follow-ups | 🚧 Skeleton | `interview/conversational.py` exists with full architecture; endpoints currently return 501 (gated WIP) |
...
| Milestone | Status | Notes |
|-----------|--------|-------|
| `TRAINER_DECISIONS_CHECKLIST.md` filled in | 🚧 Pending | Drafted, awaiting PPC training sign-off on paths/skills/compliance |
...
| PPC training feedback round | 🚧 Pending | Schedule classroom session once smoke test + checklist done |
...
1. **Get PPC training to sign off on `TRAINER_DECISIONS_CHECKLIST.md`** — interview paths, forbidden wording, ESCO skill subset. Blocks classroom run.
...
| PPC training | AMS Vienna trainer | Key feedback partner; pilot classroom gatekeeper |
