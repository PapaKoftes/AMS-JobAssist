...
| **Layer A — occupation/skill normalization** | **SHIPPED** | `data/knowledge/skills_taxonomy.json` (curated, AMS-aligned, multilingual synonyms) + `skills/normalize.py` (offline, diacritic-folding, fuzzy). `CVDocument.normalized_skills` populated in `to_canonical()`; Tool 2 `GET /api/cohorts/{id}/skills` gives the per-cohort canonical-skill histogram. Collapses "Stapler"/"forklift"/"viličar" → one label. 11 tests. Deliberately *not* applied to the search query (the user's own term searches better). Expandable from ESCO — see `docs/ROADMAP_ESCO_MEINAMS.md`. |
| **Layer C — offline labour-market insight** | **BLOCKED (data not obtainable here)** | Would bundle a `data.gv.at` snapshot ("N open positions for your job in your Bundesland"). The `data.gv.at` CKAN API returns HTML not JSON from this environment, so the real dataset URL/license could not be confirmed — and **fabricating numbers is not acceptable**. Ready to build the moment the real CSV/JSON distribution is in hand: the loader follows the bundled-`knowledge.py` pattern (frozen-safe). **Needs:** the confirmed `data.gv.at` "Offene Stellen nach Berufen" distribution URL + license. |
| **Layer D — consented live in-app match** | **BLOCKED (API + ToS unconfirmed)** | `jobs.ams.at` has a public JSON API (its SPA uses it) but it is undocumented/unofficial; using it programmatically raises stability + **ToS/legal** questions, and no browser was connected here to confirm the endpoint. Shipping an unconfirmed live fetch of user-derived data would violate both the offline-by-default guarantee's spirit and the "Datenschutz needs clear acceptance" rule. The full design (consent dialog, `*.ams.at`-scoped `temporarily_allow_network()`, audit log, default-off, local ranking via `ats.py`) stands ready. **Needs:** (1) a browser session to confirm the endpoint, (2) confirmation that programmatic use is permitted. Until both, the **Tier 0 deep-link already lets the participant search AMS**, and the paste-back analyzer already ranks a chosen posting locally — so the CV→jobs→match loop is complete without it. |
