...
- **Translate-first lift is implemented but not yet fully benchmarked.** Non-German dumps are translated to German before extraction (the root fix for the AR/RU collapse), unit-tested for wiring; the per-language F1 improvement on the gold set is being measured.
...
| Same, for non-Latin input (Arabic/Russian) | **Medium** | Translate-first added (was F1 ≈ 0); lift implemented + unit-tested, full benchmark in progress |
