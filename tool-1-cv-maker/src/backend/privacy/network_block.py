...
def temporarily_allow_network():
    ...
    operation: downloading the local AI model from HuggingFace.

    Offline mode is enforced by monkey-patching socket / urllib / http.client.
    The model download is the single legitimate outbound request in the app,
    and it only runs when the user explicitly clicks "download model". This
    context manager restores the original network primitives for the duration
    of the download and re-installs the block afterwards.

    Scope/safety: this is a single-user localhost tool; the window is the
    user-triggered download only. If offline mode was never enabled, this is a
    no-op.
    """
    nb = _network_blocker
    if not nb._blocked:
    ...
    # Restore the real (pre-patch) primitives.
